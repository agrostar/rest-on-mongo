...
module.exports.authRoutes = (routes, jwtKey) => {
  ...
  routes.get('/documents', async function (req, res) { // /:user_id/:prop
  ...
  routes.post('/documents', async function (req, res) { // /:user_id/:prop

    res.send(401);
  });
  routes.delete('/documents/:id?', async function (req, res) { // /:user_id/:prop
    res.send(401);
  });
  routes.patch('/documents/:type', passport.authenticate('jwt', { session: false }), async function (req, res) { // /:user_id/:prop
    // res.send('hello world')
    ...
    if(user.type == 'admin'){
      const collection = req.db.collection('documents');
      const docs = await collection.findOne({type: req.params.type});
      console.log(req.user);
      console.log(req.body);
      const response = await collection.updateOne({_id: docs._id}, { $set: req.body });
      res.send({ response });
    }else{
      res.send(401);
    }
