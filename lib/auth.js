...
module.exports.authenticate = () => {
  return [
     function(req, res, next) {
        console.log('process?.env?.EXCLUDE: ', process?.env?.EXCLUDE);
        if (typeof process?.env?.EXCLUDE === 'string') {
          console.log('process?.env?.EXCLUDE splited: ', process?.env?.EXCLUDE.split(' '));
          if (process?.env?.EXCLUDE.split(' ').indexOf(req.url) >= 0) { // === '/users?type=chef') {
            next();
          } else {
