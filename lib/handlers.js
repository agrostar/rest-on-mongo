...
function transformFilesInObj (item, collectionName) {
  for (const key in item) {
    if (Object.hasOwnProperty.call(item, key) && key.indexOf('_type') > 0) {
      const base64Key = key.replace('_type', '');
      // data:image/jpeg;base64
      let type = item[key].split(':')[1].split('/')[1].replace(';base64', '');
      let file = item[key].split(':')[1].split('/')[0];
      let url = '/file/' + collectionName + '/' + item._id + '/' + file + '/' + type + '/' + base64Key;
      item[base64Key + '_linked'] = url;
      delete item[base64Key];
    }
  }
}
