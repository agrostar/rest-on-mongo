...
function transformSingleElementFileToObj(element, arrayKey, item_id, collectionName){
  if (Object.hasOwnProperty.call(element, arrayKey) && arrayKey.indexOf('_type') > 0) {
    ...
    let type = element[arrayKey].split(':')[1].split('/')[1].replace(';base64', '');
    let file = element[arrayKey].split(':')[1].split('/')[0];
    let url = '/file/' + collectionName + '/' + item_id + '/' + file + '/' + type + '/' + base64Key;
